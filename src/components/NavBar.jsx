...
const NavBar = ({ classname }) => {
  ...
  return (
    <div>
      <div
        className={`${classname} bg-background justify-between px-8 flex xl:justify-center xl:relative `}
      >
        <img
          src={tfa_logo}
          alt=""
          className="h-12 self-center sm:h-auto mt-2 xl:absolute xl:left-8 xl:top-1"
        />
        ...
        <div className="flex">
          <Button
            classname={`bg-green text-white mr-8 w-32 mt-4 xl:absolute xl:right-0 hover:bg-[#29856c]`}
            text={`Donate`}
          />
          <FaBars
            className="block xl:hidden mt-8 h-8 w-8"
            onClick={handleClick}
          />
        </div>
      </div>
